...
public class UserController {
    ...
    @ApiOperationSupport(order = 6, author = "刘紫璇")
    @ApiOperation("用户状态")
    @PostMapping("status")
    ...
    @ApiOperation("用户列表")
    @ApiOperationSupport(order = 7, author = "刘紫璇")
    @GetMapping("index")
    @ApiImplicitParams({
    ...
    public CommonResult<PageResult<RbacUserVo>> index(
        ...
        PageResult<RbacUserVo> result = rbacUserService.getIndex(id,username, mobile, belongto,status,pageNum, pageSize);
